...
def sendMessage(eventDetails, subject=None, message=None):
    ...
    if subject:
        ...
        subject = 'S3 Public permission Alert for {} bucket'.format(eventDetails.get('bucketName'))
    ...
    # send message
    response = sns.publish(TopicArn='arn:aws:sns:us-east-1:052674914236:xxx',
                                   Message=message,
                                   Subject=subject)
...
def lambda_handler(event, context):
    ...
    if eventData.get('eventName') == 'PutBucketPublicAccessBlock':
        ...
        getPublicInfo = eventData.get('requestParameters').get('PublicAccessBlockConfiguration')
        # checking flags if any is false
        if not (getPublicInfo.get('RestrictPublicBuckets') and getPublicInfo.get('RestrictPublicBuckets') and getPublicInfo.get('RestrictPublicBuckets') and getPublicInfo.get('RestrictPublicBuckets')):
    ...
    if eventData.get('eventName') == 'PutBucketAcl':
        ...
        grantDetails = eventData.get('requestParameters').get('AccessControlPolicy').get('AccessControlList').get('Grant')
        # setting flag
        flag = False
        ...
        if len(grantDetails) > 1:
            ...
            for grant in grantDetails[1:]:
                ...
                if grant.get('Grantee').get('URI') and (grant.get('Grantee').get('URI') in [allUserGroup, authUserGroup]):
                    # checking if any of this permission exist
                    if grant.get('Permission') in ["READ", "WRITE", "READ_ACP", "WRITE_ACP", "FULL_CONTROL"]:
                        # setting flag true
                        flag = True
