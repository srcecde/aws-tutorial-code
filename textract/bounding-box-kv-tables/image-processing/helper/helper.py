...
async def drawBoundingBox(image, height, width, box, color_code, thickness):
    ...
    cv2.rectangle(image, (left, top), (int(left + (width * box["Width"])), int(top + (height * box["Height"]))), color_code, thickness)
    ...
    for block in response["Blocks"]:
        if block['BlockType'] == "KEY_VALUE_SET":
            if block['EntityTypes'][0] == "KEY":
                await drawBoundingBox(image, height, width, block["Geometry"]["BoundingBox"], (0, 0, 255), 1)
            else:
                await drawBoundingBox(image, height, width, block["Geometry"]["BoundingBox"], (0, 255, 0), 1)
            
        if block['BlockType'] == 'TABLE':
            await drawBoundingBox(image, height, width, block["Geometry"]["BoundingBox"], (255, 0, 0), 2)
        
        if block['BlockType'] == 'CELL':
            await drawBoundingBox(image, height, width, block["Geometry"]["BoundingBox"], (0, 255,255), 1)
